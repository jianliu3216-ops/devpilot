...
## 重点功能：项目知识库（project-knowledge-base）

> 这是 Autopilot 流程的**核心基础能力** — 让 AI 快速理解你的历史项目，是所有后续阶段的依赖。

### 它做什么？

对于已有代码的历史项目，AI 需要先理解项目结构和编码风格才能写出匹配的代码。`project-knowledge-base` 自动扫描项目，生成结构化知识文档：

- **架构概览**：项目分层、模块依赖关系、数据流向
- **模块字典**：每个模块的职责、入口、导出、依赖
- **可复用资产索引**：工具函数、公共组件、常量/配置
- **编码规范提取**：命名约定、错误处理模式、日志风格
- **重构建议**：死代码、循环依赖、结构优化点
...
| 场景 | 操作 |
|------|------|
| 历史项目接入 Autopilot | `生成知识库，目标项目：D:\my-project` |
| 新项目首次使用 | 同上，AI 自动创建空白知识库作为后续增量基础 |
| 代码有大改后同步 | `更新知识库`（增量更新，不全量重扫） |

### 输入格式

```
生成知识库，目标项目：D:\projects\my-project
```

或直接 Skill 命令：

```
/project-knowledge-base --target "D:\projects\my-project"
```
...
└── docs/
    └── knowledge-base/
        ├── PROJECT_KNOWLEDGE_BASE.md    # 项目架构总览（后续阶段必读）
        └── PROJECT_KNOWLEDGE_DETAIL.md  # 深度分析（复杂项目自动生成）
```

### 关键特性

- **一次生成，全流程复用**：后续需求分析/PRD/设计/编码 全部自动带上知识库，Agent 不需要重复扫描源码
- **增量更新**：需求完成后只更新变更涉及的章节，不全量重扫
- **CodeGraph 协同**：安装 CodeGraph 后 `codegraph build` 预生成依赖图，大幅提升模块分析准确性

详见 `skills/project-knowledge-base/` 目录下完整文档。
