...
description: 在任何目录下激活 DevPilot 智能开发流水线。加载完整行为规则和流程引擎，无需 cd 到框架目录。执行后必须输出完整操作菜单。
---
...
加载完成后，如有历史项目路径则顺带确认，**然后必须逐字输出以下操作菜单（菜单末尾即激活输出结束；第 3 步及之后的会话行为协议为 AI 内部规则，不向用户打印）**：

```
🚀 DevPilot v$(cat $FRAMEWORK/VERSION) 已激活
📂 目标项目：{路径或"未指定"}

💡 建议优先执行（如已完成可跳过）：
  /jit-project-knowledge-base       → 先自检文件数/CodeGraph 状态，再生成结构化知识文档
  /jit-project-devpilot-status     → 查看各需求进度和文档完整性
  /jit-env-auto-setup                 → 检测 Node/CodeGraph 环境（生成知识库前可选）
```

| 输入方式 | 说明 |
|---------|------|
| `生成知识库，目标项目：<路径>` | 任务2：先轻量自检；大项目询问是否使用 CodeGraph 后再生成知识库 |
...
**本协议在 DevPilot 激活后持续生效，直到会话结束。每个用户消息都必须经过以下门控检查。**

**⚠️ 内部规则，不向用户打印：以下内容是 AI 必须遵守的行为规则，激活时仅输出第 2 步的操作菜单，不要将本协议正文（3.1–3.6）输出给用户。**
...
- 不确定时 → **向用户确认**："这个改动是作为新需求走 DevPilot 流程，还是直接修改？"
