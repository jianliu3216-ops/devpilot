...
无需特殊安装，直接在对话中：
```
请先读取 ./jit-project-knowledge-base-skill/SKILL.md，然后按照这个 Skill 的流程分析当前项目，生成 PROJECT_KNOWLEDGE_BASE.md
```

如果想全局安装：
```bash
# 创建目录
mkdir -p ~/.claude/skills/jit-project-knowledge-base

# 复制文件
cp project-knowledge-base-skill/* ~/.claude/skills/jit-project-knowledge-base/
```

使用时：
```
请读取 ~/.claude/skills/jit-project-knowledge-base/SKILL.md，然后分析当前项目，生成 PROJECT_KNOWLEDGE_BASE.md
```
