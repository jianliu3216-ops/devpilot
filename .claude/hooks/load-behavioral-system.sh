...
# Self-locating: works from any directory via absolute paths

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
AUTOPILOT_HOME="$(cd "$SCRIPT_DIR/../.." && pwd)"

echo "✅ Autopilot 智能流水线 v1.5 — 就绪"

# Critical: Global Decision Engine (auto-delegation, always active per DECISION.md)
cat "$AUTOPILOT_HOME/.claude-collective/DECISION.md"

# Critical: Autopilot pipeline rules (flow triggers, change levels, output paths)
cat "$AUTOPILOT_HOME/.claude-collective/cicd-rules.md"
